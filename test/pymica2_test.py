...
class TestPyMica(unittest.TestCase):
    ...
    def test_init_wrong_variables_files(cls):
        config = {'mlr': {'resolution': 270,
                           'interpolation_bounds': [260000, 4480000,
                                                    530000, 4750000],
                            "clusters": "None",
                            "variables_files": {"altitude": "test/data/tifs/altitude.tif",
                                               "d_coast": "test/data/tifs/d_coast_2.tif"},
                            "EPSG":25831}}
    ...
    def test_interpolate_check_data(self):
        ...
        # Test que el fitxer no existeix
        with self.assertRaises(FileNotFoundError) as cm:
            inst = PyMica('id3d', 'test/data/config_test.json')
            inst.interpolate(data_file="aaaa")
        self.assertEqual('Wrong data file path.', str(cm.exception))


        with self.assertRaises(json.decoder.JSONDecodeError) as cm:
            inst = PyMica('id3d', 'test/data/config_test.json')
            inst.interpolate(data_file="test/data/sample_data_err.json")
        self.assertEqual("Expecting ',' delimiter: line 1 column 3474 (char 3473)", str(cm.exception))

        # Fem test per les lat
        data_dict=[{"id": "C6", "value": 6.4, "altitude": 264, "lon": 0.95172, "lati": 41.6566, "dist": 0.8583929293407604}, 
                   {"id": "C7", "value": 5.6, "altitude": 427, "lon": 1.16234, "lat": 41.66695, "dist": 0.8387222708681318}]
        ...
        # Fem test per les lon
        data_dict=[{"id": "C6", "value": 6.4, "altitude": 264, "loni": 0.95172, "lat": 41.6566, "dist": 0.8583929293407604}, 
                   {"id": "C7", "value": 5.6, "altitude": 427, "lon": 1.16234, "lat": 41.66695, "dist": 0.8387222708681318}]
        ...
        # Fem test per les altures
        data_dict=[{"id": "C6", "value": 6.4, "alti": 264, "lon": 0.95172, "lat": 41.6566, "dist": 0.8583929293407604}, 
                   {"id": "C7", "value": 5.6, "altitude": 427, "lon": 1.16234, "lat": 41.66695, "dist": 0.8387222708681318}]
        ...
        #Fem test per "id"
        data_dict=[{"ide": "C6", "value": 6.4, "altitude": 264, "lon": 0.95172, "lat": 41.6566, "dist": 0.8583929293407604}, 
                   {"id": "C7", "value": 5.6, "altitude": 427, "lon": 1.16234, "lat": 41.66695, "dist": 0.8387222708681318}]
        ...
        # Fem test per "value"
        data_dict=[{"id": "C6", "temp": 6.4, "altitude": 264, "lon": 0.95172, "lat": 41.6566, "dist": 0.8583929293407604}, 
                   {"id": "C7", "valor": 5.6, "altitude": 427, "lon": 1.16234, "lat": 41.66695, "dist": 0.8387222708681318}]
        ...
        # Fem test variables_file (parametres a config ha de tenir variables a data)4

        # Fem test per "value"
        data_dict=[{"id": "C6", "value": 6.4, "altitude": 264, "lon": 0.95172, "lat": 41.6566, "dist": 0.8583929293407604}, 
                   {"id": "C7", "value": 5.6, "altitude": 427, "lon": 1.16234, "lat": 41.66695, "dist": 0.8387222708681318}]
        ...
        with self.assertRaises(KeyError) as cm:
            inst=PyMica('mlr', 'test/data/config_test.json')
            inst.interpolate(data_file='test/data/sample_data_test_keys.json')
        self.assertEqual('Some variables missing in C6',
                         cm.exception.args[0])