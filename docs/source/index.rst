...
and inverse of the distance procedure. Performance of the interpolation was assessed 
...
the methodology proposed is that there is not a defined number of clusters in how stations 
are grouped or divided but several number of clusters are considered and the one that 
performs the best is selected. The MICA system can be divided in two parts. The first 
one involves the definition of clusters and the explanatory variables that are going to 
be used in the MLR calculations. The second one is based on the choice of the best number
of clusters and on obtaining the final variable of interest field.
